...
// INSTPAT条目核心：opcode functs3 functs7 ；源寄存器，目标寄存器，以及立即数利用 ? 表示 
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = (signed)src1 < (signed)src2);
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (unsigned)src1 < (unsigned)src2);
