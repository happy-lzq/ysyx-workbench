...
//==================================== 立即数运算指令 ===================================================================
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & 0x1F));  // 注意 imm 低 5 位有效
  INSTPAT("??????? ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm & 0x1F));  // 逻辑右移
  INSTPAT("??????? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (signed)src1 >> (imm & 0x1F)); 
