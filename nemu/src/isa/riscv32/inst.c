...
// INSTPAT条目核心：opcode functs3 functs7 ；源寄存器，目标寄存器，以及立即数利用 ? 表示 
  INSTPAT_START();
// =================================== 长立即数指令 ==================================================================
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm);  // imm 已左移12位
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);

// =================================== 加载和存储指令 ==================================================================
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = (int8_t)Mr(src1 + imm, 1));       // 有符号字节类型转化  
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = (int16_t)Mr(src1 + imm, 2));      // 有符号字节类型转化
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));               // 隐式零扩展
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));               // 隐式零扩展
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));               // 原样拷贝
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));                 // 内部截断
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));                 // 原样写入
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));                 // 内部截断
...
//==================================== 立即数运算指令 ===================================================================
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", sll    , R, R(rd) = src1 << src2);                   // x[rd] = x[rs1] ≪ x[rs2]
  INSTPAT("0000000 0???? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & 0x1F));           // x[rd] = x[rs1] ≪ shamt 当且仅当shamt[5]=0,有效
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srl    , R, R(rd) = src1 >> src2);                    // x[rd] = (x[rs1] ≫𝑢 x[rs2])
  INSTPAT("0000000 0???? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm & 0x1F)); 
  INSTPAT("0100000 0???? ????? 101 ????? 00100 11", srai   , I, R(rd) = (signed)src1 >> (imm & 0x1F)); 
...
// ==================================== 算术运算指令 =====================================================================
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = ((src2 == 0 ) ? ~ 0 : 
                                                                        ((src1 == 0x80000000 && src2 == -1) ? 0x80000000 : 
                                                                        (int32_t) src1 / (int32_t) src2 )));  
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2 == 0 ) ? ~0: src1 / src2); 
