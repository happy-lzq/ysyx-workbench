...
我们在推流 4096 字节的 PCM 时，使用了一段极其优美的非阻塞轮询预测：
```c
int audio_count = io_read(AM_AUDIO_STATUS).count;
int free_space = max_bufsz - audio_count;
if (free_space >= AUDIO_CHIP) { 
    io_write(AM_AUDIO_PLAY, play); 
}
```

* **AM层的强制拦截流控（坚决被淘汰的保守派）**：
  如果我们在没做 `if` 条件判断的时候去疯狂用 `io_write()` 填数据，且底层驱动的盘（64KB）已经被装满了！AM 驱动别无选择：它只会用阻塞循环把你的程序卡在其中慢慢漏。你的 C 代码被按死在这里几秒，屏幕视频几秒内会原地暂停，画面极度滞后！
* **软件层的主动避让流控（大局为重的走位）**：
  上述这段先去摸底查 `reg_count` 的逻辑告诉我们：发现“饭盘”快满了，那这次我们就先不发出了。大循环越过这个 if ，迅速去判断“现在时间到没到要发视频”。
  它从根本上废掉了任何死循环或者等待。这就是非阻塞 I/O (Non-Blocking I/O) 的鼻祖，真正用一层代码就将异声卡和显卡无痛分时协同在一起。
...
* **音频：饥饿与补货（忽视时间维度）**
  硬件声卡自带晶振（固定 44100 滴答），会以极其苛刻均匀的速率在后台偷吃缓存池。我们的任务：通过疯狂高速的 `while(1)` 监测空余，只要空出来一块，瞬间丢下一块。喂得勤，声波就自然随着现实时间走。
* **视频：打卡与冷宫（被绝对时间约束）**
  系统以 `uint64_t now_us` 为唯一绝对指针，由于宏 `#define FRAME_T (1000000 / 30)`，一帧等于 33333 绝对微秒。
  靠着判定：`expected_frame = (now - start) / FRAME_T` 是否越过了此时已画完了的 `current_frame` ？如果越过，才代表闹钟长鸣，将那一阵对应的像素推到 `AM_GPU_FBDRAW` 里。（一旦 CPU 如果太忙导致延误了呢？那下一次判断里，期望值会一次性爆跨，代码连跳数帧以强行保齐时空刻度！）
...
2. **“视频斜边与撕裂”：切记内存跨度的单位是字节**
   `rgb_ptr` 本质为了规避强转，是以 `uint8_t` 进行定义的！在刷一帧之后的指针向下一帧的物理偏移时，
   `rgb_ptr += FRAM_W * FRAM_H;` 是错的，那只是一帧中的 1/4 地址。需要牢记你的底料是 bgra格式 的 32 位：
   **正确：`rgb_ptr += FRAM_W * FRAM_H * 4;`**
3. **“声音一波一波拉风箱抽搐”：不该等待不要死等**
   曾天真地在一帧后附言 `while (io_read(AM_AUDIO_STATUS).count > 0);` 强制等待声音播光再走。这种愚蠢等待使原本平滑延绵流动的“长河”被割碎变成脉冲块水杯倒水，导致刺耳爆频。**非阻塞不需要这行等待，请删掉！**
4. **编译目录脱节（不在家报错）与头文件引用不适**
   如果是将其自身视为一个**独立的运行程序模块**进行 `make ARCH=riscv32-nemu run` 测试时，**切记不可引用包含不该引用的其测试工程独占的头文件库（如 `amtest.h` 和不规范 `common.h`）！**，你独立的时候只有 `klib.h` 、`am.h` 以及自带的一些类型宏 `<stdint.h>` 为伍，而你以前宏大的 `Assert()` 必须全替换掉标准的 `assert()` 或者 `panic()`。
5. **宿主机的 PMEM 撑死炸膛**
   当加入的音频和视频合并资源超过 128 MB（NEMU物理初始内存一般限制值界限之内），启动 NEMU 的那一个瞬间便会报 `Assertion 'ret == 1' failed` 并核心已转储。注意用完把多余的其他 payload 注释掉或缩小资源片段以适配资源池。
