...
#### 1. 命令解析框架（sdb.c）
- 用户在 SDB 命令行输入命令，比如：`p 1+2*3`
- SDB 主循环（`sdb_mainloop`）读取用户输入，分割为命令和参数。
- 遍历 `cmd_table[]`，找到对应命令（如 `p`），调用其处理函数（如 `cmd_p(args)`）。

#### 2. 注入表达式求值命令
- 在 `cmd_table[]` 中添加 `p` 命令，并实现 `cmd_p` 函数。
- `cmd_p` 的作用是：接收表达式字符串参数，调用表达式求值模块，输出结果。

#### 3. 执行 token 解析（expr.c）
- `cmd_p` 调用 `expr(args, &success)`。
- `expr` 首先调用 `make_token(args)`，用正则表达式把输入字符串分割成一个个 token。
  - 例如 `1+2*3` 会被分成：`TK_NUM`（1）、`TK_PLUS`（+）、`TK_NUM`（2）、`TK_MUL`（*）、`TK_NUM`（3）。
- `make_token` 会把每个 token 的类型和内容保存到 `tokens[]` 数组。

#### 4. 表达式求值（expr.c）
- `expr` 在完成 token 解析后，进入表达式求值阶段。
- 通常用递归下降法或栈式算法，按照运算符优先级和括号嵌套，递归计算表达式的值。
- 计算结果返回给 `cmd_p`。

#### 5. 输出结果
- `cmd_p` 根据 `expr` 返回的结果，输出到命令行。
- 用户看到表达式的计算结果。
